...
判定閾値=0.9 の場合、多くのデータは未知としてフィルタリングされるが、Refine 適用後は、差分としては、未知が 35 件減少し、その内訳は正答の 35 件増加に対応している。
既知と判定したデータに対する正答率は 99.9476% から 99.9481% とわずかに上昇している。
一方、判定閾値=0.5 の場合、Refine 処理をすることで、正答数が5件減少、未知判定数が1件減少し、誤答率が6件増加している。
一致判定値=0.7 の場合は、Refine 処理により未知判定数が15減少し、正答数が10件、誤答数が5件増加している。
Refine 処理自体が与える影響については更に調査が必要だと考えている。
...
| True | 0.5 | 0.9594 | 0.0104 | 0.0302	| 0.969482619240097 | 9594 | 104 | 302 | - |
| True | 0.7 | 0.9322 | 0.0539 | 0.0139 | 0.985308106965437 | 9322 | 539 | 139 | - |
| True | 0.9 | 0.3849 | 0.6149 | 0.0002 | 0.9994806543754868 | 3849 | 6149 | 2 |  - |
